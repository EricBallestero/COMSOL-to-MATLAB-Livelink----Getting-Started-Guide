...
disp(' -- Probing')
%-------------------------------------------------------------------------%
Data.ReScatt = mphinterp(model,Probe.Expression.ReScatt,'coord',Probe.Coordinates);
Data.ImScatt = mphinterp(model,Probe.Expression.ImScatt,'coord',Probe.Coordinates);
Data.TotScatt = Data.ReScatt + 1i*Data.ImScatt;
%-------------------------------------------------------------------------%
%% SAVE
disp(' -- Save model')
%-------------------------------------------------------------------------%
mphsave(model,[File.Path,'/',File.Tag,File.Extension]);
out = model;
disp(' -- DONE')
end
